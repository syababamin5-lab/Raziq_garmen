...
import { getInvoiceDetails } from '../../api/penjualanApi'
...
export default function InvoiceDetailModal({ isOpen, onClose, invoice }) {
  ...
  useEffect(() => {
    if (isOpen && invoice?.no_invoice) {
      setLoading(true)
      getInvoiceDetails(invoice.no_invoice)
        .then(res => {
          if (res.success) {
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100">
        ...
        <div className="bg-emerald-800 p-8 text-white relative">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-2xl font-black tracking-tighter uppercase">Rincian Invoice</h2>
              <p className="text-emerald-100/80 font-bold text-sm tracking-widest">{invoice?.no_invoice}</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8">
          <div className="grid grid-cols-2 gap-8 mb-8 bg-slate-50 p-6 rounded-3xl border border-slate-100">
            <div>
              ...
              <p className="text-sm font-black text-slate-800">{invoice?.nama_produk || invoice?.nama_customer}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tanggal Transaksi</p>
              <p className="text-sm font-black text-slate-800">{invoice?.tanggal}</p>
            </div>
          </div>
                  ...
                  details.map((item, idx) => (
                    <tr key={idx} className="group">
                      ...
                      <td className="py-4 text-right text-xs font-bold text-slate-600">
                        {formatRp(item.harga_per_lusin)}
                      </td>
                      <td className="py-4 text-right text-sm font-black text-emerald-700">
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <span>{formatRp((invoice?.total_tagihan || 0) + (invoice?.diskon || 0))}</span>
              </div>
              <div className="flex justify-between items-center text-[10px] font-bold text-red-500 uppercase tracking-widest">
                <span>Diskon</span>
                <span>-{formatRp(invoice?.diskon || 0)}</span>
              </div>
              <div className="flex justify-between items-center text-[10px] font-bold text-blue-600 uppercase tracking-widest border-t border-slate-50 pt-2">
                <span>Uang Muka (DP)</span>
                <span>{formatRp(invoice?.uang_muka || 0)}</span>
              </div>
            </div>

            <div className="text-right flex flex-col justify-end">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                {invoice?.status === 'SELESAI' ? 'Sisa Tagihan (LUNAS)' : 'Total Belum Dibayar'}
              </p>
              <h3 className={`text-2xl font-black leading-none mt-1 ${invoice?.status === 'SELESAI' ? 'text-emerald-700' : 'text-red-600'}`}>
                {invoice?.status === 'SELESAI' ? 'Rp 0' : formatRp((invoice?.total_tagihan || 0) - (invoice?.uang_muka || 0))}
              </h3>
              <div className="mt-3">
                 <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase ${
                    invoice?.status === 'SELESAI' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                 }`}>
