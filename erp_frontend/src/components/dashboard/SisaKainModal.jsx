...
import { getDetailKain } from '../../api/dashboardApi'
import { formatRp } from '../../utils/formatters'

export default function SisaKainModal({ isOpen, onClose }) {
  const [data, setData] = useState([])
  const [loading, setLoading] = useState(false)
  const [expandedModel, setExpandedModel] = useState(null)
  const [grandTotal, setGrandTotal] = useState({ kg: 0, nilai: 0 })
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-3xl overflow-hidden animate-in zoom-in-95 duration-300 border border-slate-100 flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="bg-[#92400E] p-8 text-white relative flex-shrink-0">
          ...
          <div className="flex items-center gap-5">
            <div className="w-16 h-16 bg-white/10 rounded-3xl flex items-center justify-center backdrop-blur-md border border-white/20 shadow-inner">
              <span className="material-symbols-rounded text-3xl">texture</span>
            </div>
            <div>
              <h2 className="text-2xl font-black tracking-tighter uppercase">Rincian Sisa Kain</h2>
              <p className="text-amber-200/80 font-bold text-sm tracking-widest">Detail Per Jenis Bahan</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 bg-slate-50 flex-1 overflow-y-auto custom-scrollbar">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center gap-4">
              <span className="material-symbols-rounded animate-spin text-4xl text-amber-500">refresh</span>
              <p className="text-slate-400 font-bold animate-pulse uppercase tracking-widest text-xs">Memuat data gudang kain...</p>
            </div>
          ) : data.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center gap-4">
              <span className="material-symbols-rounded text-6xl text-slate-200">inventory_2</span>
              <p className="text-slate-400 font-bold italic">Belum ada data sisa kain.</p>
            </div>
          ) : (
              ...
              {data.map((modelGroup, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div 
                    ...
                    className="p-5 flex items-center justify-between cursor-pointer hover:bg-amber-50/50 transition-colors group"
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${expandedModel === idx ? 'bg-amber-600 text-white shadow-md shadow-amber-200' : 'bg-slate-100 text-slate-500 group-hover:bg-amber-100 group-hover:text-amber-600'}`}>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">{modelGroup.jenis}</h3>
                          <span className="bg-slate-100 text-slate-500 text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest border border-slate-200">
                            {modelGroup.items.length} Roll/Varian
                          </span>
                        </div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Kategori Jenis Kain</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="flex items-baseline justify-end gap-1.5">
                        <span className="text-lg font-black text-amber-700">{modelGroup.total_stok_kg.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      ...
                      <table className="w-full text-left">
                        <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200">
                          <tr>
                            ...
                            <th className="pb-3 px-2 text-right">Stok (Kg)</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {modelGroup.items.map((item, itemIdx) => (
                            <tr key={itemIdx} className="hover:bg-amber-50/30 transition-colors">
                              <td className="py-3 px-2">
                                <p className="text-xs font-black text-slate-700 leading-tight mb-0.5">{item.nama_barang}</p>
                                <p className="text-[10px] font-bold text-amber-500 uppercase">{item.kode_sku}</p>
                              </td>
                              <td className="py-3 px-2 text-right">
                                <div className="flex items-baseline justify-end gap-1">
                                  <span className="text-sm font-black text-slate-800">{item.stok_kg.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        ...
        <div className="bg-white p-6 border-t border-slate-100 flex-shrink-0 flex items-center justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-black text-amber-800">{grandTotal.kg.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
