...
def export_invoice_pdf(header_inv, detail_items, terbilang_teks):
    ...
    if (getattr(header_inv, 'diskon', 0) or 0) > 0:
        pdf.cell(120, 8, '', 0, 0) 
    ...
    if "Tempo" in header_inv.metode_bayar:
        # Tampilkan informasi jatuh tempo (contoh: +30 hari dari tanggal invoice)
        try:
            tgl_jt = header_inv.tanggal + datetime.timedelta(days=30)
            tgl_jt_str = tgl_jt.strftime('%d %b %Y')
        except:
            tgl_jt_str = "Sesuai Kesepakatan"
            
        pdf.set_font('Arial', 'B', 9)
        pdf.cell(80, 5, 'Informasi Pembayaran (Piutang/Tempo):', 'LTR', 1, 'L')
        pdf.set_font('Arial', '', 9)
        pdf.cell(80, 5, f'Jatuh Tempo: {tgl_jt_str}', 'LR', 1, 'L')
        pdf.cell(80, 5, 'Mohon lakukan pelunasan tepat waktu.', 'LR', 1, 'L')
        pdf.cell(80, 5, '', 'LBR', 1, 'L')
        
    elif "Transfer" in header_inv.metode_bayar:
        pdf.set_font('Arial', 'B', 9)
        pdf.cell(80, 5, 'Pembayaran ditujukan kepada (Transfer Bank):', 'LTR', 1, 'L')
        pdf.set_font('Arial', '', 9)
        ...
        # Tunai
        pdf.set_font('Arial', 'B', 9)
        pdf.cell(80, 5, 'Metode Pembayaran: TUNAI', 'LTR', 1, 'L')
        pdf.set_font('Arial', '', 9)
        pdf.cell(80, 5, 'Lunas Dibayar.', 'LR', 1, 'L')
        pdf.cell(80, 5, 'Terima kasih atas kepercayaannya.', 'LR', 1, 'L')
        pdf.cell(80, 5, '', 'LBR', 1, 'L')
