...
async def submit_pembelian_bahan(payload: schemas.PembelianBahanRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        total_transaksi = sum(i.qty * i.harga for i in payload.items)
        ...
        # 4. Jurnal Accounting
        if total_baku > 0:
            db.add(models.JurnalUmum(tanggal=datetime.datetime.now(), kode_akun="12110", nama_akun="Persediaan Kain", keterangan=f"Beli {no_po}", debit=total_baku, kredit=0))
        if total_penolong > 0:
            db.add(models.JurnalUmum(tanggal=datetime.datetime.now(), kode_akun="12120", nama_akun="Persediaan Penolong", keterangan=f"Beli {no_po}", debit=total_penolong, kredit=0))

        if payload.metode_pembayaran == "Kas Tunai":
            db.add(models.JurnalUmum(tanggal=datetime.datetime.now(), kode_akun="11110", nama_akun="Kas Tunai", keterangan=f"Beli {no_po}", debit=0, kredit=total_transaksi))
        elif payload.metode_pembayaran == "Transfer Bank":
            db.add(models.JurnalUmum(tanggal=datetime.datetime.now(), kode_akun="11120", nama_akun="Bank", keterangan=f"Beli {no_po}", debit=0, kredit=total_transaksi))
        else: # Utang
            db.add(models.JurnalUmum(tanggal=datetime.datetime.now(), kode_akun="21110", nama_akun="Utang Usaha", keterangan=f"Beli {no_po}", debit=0, kredit=total_transaksi))
            supp.saldo_utang = (supp.saldo_utang or 0) + total_transaksi
...
def get_pembelian_list(db: Session = Depends(get_db)):
    data = db.query(models.HeaderPembelian).order_by(models.HeaderPembelian.tanggal.desc()).limit(50).all()
    return schemas.APIResponse(success=True, message="Success", data={"list": data})
...
def get_pembelian_details(no_po: str, db: Session = Depends(get_db)):
    ...
    return schemas.APIResponse(success=True, message="Success", data={"details": data})
...
def submit_opex(payload: schemas.OpexRequest, db: Session = Depends(get_db)):
    try:
        ...
        nama_akun_kredit = "Kas Tunai" if payload.sumber_dana == "Kas Tunai" else "Bank"
...
def submit_aset(payload: schemas.AsetRequest, db: Session = Depends(get_db)):
    try:
        ...
        if payload.sumber_dana == "Modal Awal (Khusus Aset Lama)":
            ...
            nama_kredit = "Kas Tunai" if payload.sumber_dana == "Kas Tunai" else "Bank"
            
        db.add(models.JurnalUmum(tanggal=waktu_aset, kode_akun=payload.kode_akun_aset, nama_akun=payload.nama_akun_aset, keterangan=payload.nama_barang, debit=payload.nominal, kredit=0))
...
def get_aset_summary(db: Session = Depends(get_db)):
    try:
        jurnals = db.query(models.JurnalUmum).filter(
            models.JurnalUmum.kode_akun.in_(["13110", "13210", "13310", "13410"]),
...
def submit_penyusutan(db: Session = Depends(get_db)):
    try:
        ...
        if susut_kendaraan > 0:
            db.add(models.JurnalUmum(tanggal=waktu_susut, kode_akun="62170", nama_akun="Beban Penyusutan Kendaraan Kantor", keterangan=f"Susut Otomatis {bulan_ini}", debit=susut_kendaraan, kredit=0))
        ...
        if susut_inventaris > 0:
            db.add(models.JurnalUmum(tanggal=waktu_susut, kode_akun="62170", nama_akun="Beban Penyusutan Furniture Kantor", keterangan=f"Susut Otomatis {bulan_ini}", debit=susut_inventaris, kredit=0))
...
def submit_retur_pembelian(payload: schemas.ReturPembelianRequest, db: Session = Depends(get_db)):
    try:
        # Implementation for Retur Pembelian
        # Requires validating PO, restoring stock, adjusting journal entries
        return schemas.APIResponse(success=False, message="Retur Pembelian belum diimplementasikan di backend baru.")
    except Exception as e:
        db.rollback()
        return schemas.APIResponse(success=False, message=str(e))
